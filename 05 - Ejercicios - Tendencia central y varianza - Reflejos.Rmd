...
Los resultados se registraron en la planilla "REFLEJOS.csv" que se encuentra en la carpeta "datos".
...
data_tidy <- data %>%
  ...
  mutate(DISTANCIA = as.numeric(gsub(",", ".", VALOR)), TIEMPO = sqrt(2 * DISTANCIA / 981)) %>% select(ORDEN, NOMBRE, DISTANCIA, TIEMPO)
...
Conservamos la información de ORDEN en el que se obtuvieron los datos para poder verificar si a lo largo del experimento hubo cambios en la reacción de cada sujeto.
...
Se propone volver a la línea 76 y descomentarla para descartar los datos en que la DISTANCIA sea menor a 30.
...
grafDistancia1 <- ggplot(data_tidy, aes(x = DISTANCIA, fill = NOMBRE)) +
  ...
  geom_vline(data = data_tidy %>% group_by(NOMBRE) %>% summarise(mean_value = mean(DISTANCIA)),
             aes(xintercept = mean_value, color = NOMBRE), linetype = "dashed", size = 1) +
...
ggplot(data.frame(x = rango), aes(x = rango)) +
  stat_function(fun = dnorm, n = 30, args = list(mean = media1, sd = sd1), color = 'red') +
  stat_function(fun = dnorm, n = 30, args = list(mean = media2, sd = sd2), color = 'blue') +

...
Se puede transformar cualquier Data Frame (como los creados anteriormente) en un tibble. No es una buena práctica acostumbrarse a esto porque se pierden algunas de las ventajas como por ejemplo que las columnas de tibble pueden ser cualquier string. En este ejemplo usamos dataframeS que ya existe en R, que se llaman iris y cars. iris y cars son lo que llamamos "built-in dataset", más detalles en "03  - Built-in datasets.Rmd".
...
O cargar toda la base de datos. Esto hay que hacerlo con cuidado cuando la base es grande y dependiendo de la RAM de la computadora usada. Nótese que la función dbGetQuery() me devuelve un data frame (df), para convertirmo a tibble debo usar la función tibble() como en el caso anterior.

...
```{r}
indicesIris <- irisTidy %>% group_by(Species,Variable) %>% reframe(n = n(), media = mean(valor), q1 = q1(valor), mediana = median(valor),q2 = q2(valor), sd = sd(valor), varianza = var(valor), cv = sd/media)
indicesIris
```
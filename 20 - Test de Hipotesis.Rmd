...
## Comparación de una muestra con un valor de referencia.
...
mediaPoblacion <- 0.22 # es el valor de referencia en nuestras hipótesis
```
...
ggplot(reflejosTidy) +
  ...
  ylim(c(0, 0.25)) +
  geom_hline(yintercept = mediaPoblacion)
```
...
```{r}
mu <- mean(muestraUnaPersona$TIEMPO)
n <- nrow(muestraUnaPersona)

estadisticoDePrueba <- (mu - mediaPoblacion) / (desvioEstandarPoblacion / sqrt(n))
...
```{r}
zTestPersona <- z.test(muestraUnaPersona$TIEMPO, alternative='less', mu = mediaPoblacion, sigma.x = desvioEstandarPoblacion) # la H1 dice que la media real es menor a mu, eso se especifica con "less". Podría ser también "two.sided" o "greater" 
zTestPersona
```

```{r}
errorEstandarPoblacion <- desvioEstandarPoblacion / sqrt(n)
rango = c(min(mediaPoblacion - 3 * errorEstandarPoblacion, mu), mediaPoblacion + 3 * errorEstandarPoblacion)
ggplot(data.frame(x = rango), aes(x = rango)) +
  stat_function(fun = dnorm, n = 101, args = list(mean = mediaPoblacion, sd = errorEstandarPoblacion)) +
  geom_area(stat = 'function', fun = dnorm, args = list(mean = mediaPoblacion, sd = errorEstandarPoblacion), fill = '#ff0018', xlim = c(rango[1], mu), alpha = 0.5) +
  geom_vline(xintercept = mu) +
  geom_label(aes(x = mu, y = -7, label = round(mu,3)))  +
...
estadisticoDePrueba <- (mu - mediaPoblacion) / (desvioEstandarMuestra / sqrt(n))
...
```{r}
tTestPersona <- t.test(muestraUnaPersona$TIEMPO, alternative = "less", mu = mediaPoblacion) # la H1 dice que la media real es menor a mu, eso se especifica con "less". Podría ser también "two.sided" o "greater"
tTestPersona
```
...
Aquí vamos a comparar los tiempos de reacción de de dos personas. Como a priori no tengo ninguna razón para suponer que una de las personas tiene menor tiempo de respuesta que otra, las hipótesis se plantean como igual o distinto.

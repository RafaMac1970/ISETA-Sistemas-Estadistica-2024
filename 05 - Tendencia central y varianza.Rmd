...
```{r}
indicesMuestras <- muestrasTidy %>% group_by(muestra) %>% reframe(n = n(), min = min(valor), q1 = q1(valor), mediana = median(valor), q3 = q3(valor), Max = max(valor), media = mean(valor), varianza = var(valor), sd = sd(valor), cv = sd/media, EE = std.error(valor), "IC95-" = ic95(valor)[1], "IC95+" = ic95(valor)[2])
indicesMuestras
```

...
Canal de youtube Ingeniería elemental. Varianza y Desviación Estándar <https://www.youtube.com/watch?v=cg4kUsbjQms> 
...
### Curtosis (kurtosis en inglés)

$$
g_2={\frac{{\displaystyle \sum_{i=1}^{n}\left(x_{i}-\mu\right)^{4}}}{N\sigma^{4}}} - 3
$$
Cuando el lacurtosis es mayor que 3 quiere decir que las colas de la distribución son más pesadas que en la distribución normal.
...
```{r}
indicesMuestras <- muestrasTidy %>% group_by(muestra) %>% reframe(n = n(), min = min(valor), q1 = q1(valor), mediana = median(valor), q3 = q3(valor), Max = max(valor), media = mean(valor), varianza = var(valor), sd = sd(valor), cv = sd/media, EE = std.error(valor), asimetria = skewness(valor), curtosis = kurtosis(valor), "IC95-" = ic95(valor)[1], "IC95+" = ic95(valor)[2])
indicesMuestras
```
...
ggplot(muestrasTidy, aes(x=valor, color = muestra)) + 
  ...
  geom_vline(xintercept = indicesMuestras[[1,8]], alpha = 0.3) +
  geom_vline(xintercept = indicesMuestras[[2,8]], alpha = 0.3) +
  geom_vline(xintercept = indicesMuestras[[3,8]], alpha = 0.3) +
  geom_vline(xintercept = indicesMuestras[[4,8]], alpha = 0.3) +
  xlab("valores") + ylab("frecuencia")
```
